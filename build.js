...
import { transformerMetaHighlight, transformerMetaWordHighlight, transformerNotationDiff, transformerNotationErrorLevel, transformerNotationFocus, transformerNotationHighlight } from '@shikijs/transformers';
...
const md = MarkdownIt({
	...
	.use(
		await pluginShiki({
			...
			transformers: [transformerNotationDiff(), transformerNotationErrorLevel(), transformerNotationFocus(), transformerNotationHighlight(), transformerMetaHighlight(), transformerMetaWordHighlight()]
		})
	);
...
function processHTML(html, headings) {
	...
	const headingsFlat = headings
		.map(x => [
			{ h: 2, t: x.heading.t, s: x.heading.s },
			...x.children.map(y => {
